...
# Se escalan todas las columnas de interés juntas.
...
def create_sequences_multivariate(data, look_back):
    ...
    for i in range(len(data) - look_back):
        # Secuencia X: los últimos 'look_back' valores y sus 3 características
        X.append(data[i:(i + look_back), :])
        # Valor Y: la 'demanda_real' del siguiente punto
...
num_features = X_train.shape[2] # Ahora el modelo sabe que hay 3 features
...
y_pred_escalada = model.predict(X_test)

# Invertir el escalado. Se debe crear una matriz del tamaño original.
...
print(f"Error (MAPE) del modelo LSTM en los datos de test: {mape:.2f}%")
...
plt.plot(resultados_test.index, resultados_test['real'], label='Demanda Real (MW)', color='steelblue', linewidth=1.5)
plt.plot(resultados_test.index, resultados_test['prediccion'], label='Predicción LSTM (MW)', color='forestgreen', linewidth=2.5, alpha=0.8)
plt.title(f'Predicción de Demanda con LSTM | RMSE: {rmse:.2f} MW | MAPE: {mape:.2f}%', fontsize=18)
